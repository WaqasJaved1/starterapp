 ...
 * jsonformatter
 * 
 * Version: 0.6.0 - 2017-07-07T07:26:42.789Z
 * License: Apache-2.0
 */
...
angular.module('jsonFormatter', ['RecursionHelper', 'ngMdIcons'])
...
.directive('jsonFormatter', ['RecursionHelper', 'Location', 'JSONFormatterConfig', function jsonFormatterDirective(RecursionHelper, JSONFormatterConfig) {
  ...
  function link(scope) {
    if(typeof(scope.key)!='undefined'){
      scope.currentParents = scope.parents.concat([scope.key]);
    }
    else{
      scope.currentParents = [];
    }
    scope.editClick(['abc', 'def']);
    scope.editClicked = function(data){
      // scope.editClick({data: data});
      Location.editLocation(data);
    }
    scope.deleteClicked = function(data){
      scope.deleteClick({data: data});
    };
    scope.getParents = function(){
      return scope.currentParents;
    }
...
angular.module("jsonFormatter").run(["$templateCache", function($templateCache) {$templateCache.put("json-formatter.html","<div ng-init=\"isOpen = open && open > 0\" class=\"json-formatter-row\"><a><span ng-click=\"toggleOpen()\" class=\"toggler {{isOpen ? \'open\' : \'\'}}\" ng-if=\"isObject()\"></span> <span ng-click=\"toggleOpen()\" class=\"key\" ng-if=\"hasKey\"><span class=\"key-text\">{{key}}</span><span class=\"colon\">:</span></span> <span class=\"value\"><span ng-if=\"isObject()\"><span ng-if=\"isArray()\"><span class=\"bracket\">[</span><span class=\"number\">{{json.length}}</span><span class=\"bracket\">]</span></span></span> <span ng-if=\"!isObject()\" ng-click=\"openLink(isUrl)\" class=\"{{type}}\" ng-class=\"{date: isDate, url: isUrl}\">{{parseValue(json)}}</span></span> <span class=\"spacing\"></span> <span ng-if=\"hasKey\"><ng-md-icon ng-click=\"editClicked(currentParents)\" icon=\"mode_edit\" size=\"15\"></ng-md-icon><span class=\"spacing\"></span><ng-md-icon ng-click=\"deleteClicked(currentParents)\" icon=\"delete\" size=\"15\"></ng-md-icon></span> <span ng-if=\"showThumbnail()\" class=\"thumbnail-text\">{{getThumbnail()}}</span></a><div class=\"children\" ng-if=\"getKeys().length && isOpen\"><json-formatter delete-click=\"deleteClick(data)\" edit-click=\"editClick(data)\" ng-repeat=\"key in getKeys() track by $index\" parents=\"getParents()\" json=\"json[key]\" key=\"key\" open=\"childrenOpen()\"></json-formatter></div></div>");}]);